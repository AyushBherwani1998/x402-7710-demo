...
```
Frontend (Next.js)         Seller (Express)            Sentinel (MetaMask)
┌─────────────────┐        ┌───────────────────┐       ┌──────────────────┐
...
**Roles:**
- **Seller** — owns the protected resource, declares payment terms (`payTo` address), delegates verification and settlement to Sentinel via HTTP
- **Sentinel** — MetaMask's hosted facilitator service that verifies delegations (via simulation) and settles payments on-chain
...
3. The embedded EOA redelegates to the Sentinel facilitator with caveats: `redeemer` (only Sentinel can redeem) and `allowedCalldata` (enforces the seller's `payTo` address as the transfer recipient)
4. The full delegation chain is encoded in an x402 `payment-signature` header and sent with the request to the seller
5. Seller forwards the payment to Sentinel for verification (simulation of `redeemDelegations`)
6. Seller returns the premium data, then Sentinel settles the payment on-chain
...
# URL of the Sentinel facilitator service (default: MetaMask Sentinel)
FACILITATOR_URL=https://tx-sentinel-base-mainnet.dev-api.cx.metamask.io
...
4. Click **Grant Permission** to authorize the Sentinel facilitator for periodic USDC transfers
...
| Facilitator | [MetaMask Sentinel](https://tx-sentinel-base-mainnet.dev-api.cx.metamask.io) |
...
│   │       ├── middleware.ts    # x402 payment middleware (calls Sentinel via HTTP)
